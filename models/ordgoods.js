...
exports.add = function add(data) {
  return dbUtils.add('ordgoods', data);
}
...
function query(sql, params) {
  ...
  while (rows.next()) {
    ...
    var goodsId = types.newString();
    var goodsName = types.newString();
    var price = types.newString();
    var num = types.newInt();
    var created_at = types.newString();

    var err = rows.scan(id, oid, ordId, goodsId, goodsName, price, num, created_at);
    if (err) {
      rows.close();
      throw err;
    }
    ...
    ret.push({
      ...
      goodsId: types.stringValue(goodsId),
      goodsName: types.stringValue(goodsName),
      price: types.stringValue(price),
      num: types.intValue(num),
